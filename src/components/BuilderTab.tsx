...
export function BuilderTab({
  ...
  const targetNeeded         = builderMode === "total"
    ? Math.max(0, internalTarget - currentStats)
    : internalTarget;
      ...
      <div className="p-4 bg-[#09141f] border border-[#1a3f6e] rounded-lg space-y-4">
        <div>
          ...
          <select value={selectedAttr} onChange={e => onAttrChange(e.target.value as Attribute)}
            ...
            {ATTRIBUTES.map(attr => (
              <option key={attr} value={attr}>{ATTR_ICON[attr]} {attr}</option>
            ))}
          </select>
        </div>
            ...
            {(["total", "add"] as const).map(mode => (
              <button key={mode} onClick={() => onBuilderModeChange(mode)}
                className={`px-3 py-2 rounded text-xs font-mono transition-all ${
                  builderMode === mode ? "bg-[#00c8f0]/20 border border-[#00c8f0] text-[#00c8f0]"
          ...
          {useSlider ? (
            <div className="space-y-3">
              <input type="range" min={builderMode === "total" ? currentDisplay : 0}
                ...
                step={isPct ? 0.001 : 1} value={displayTarget}
                onChange={e => onTargetStatChange(e.target.value)} className="w-full" />
            ...
            <div className="space-y-2">
              ...
              <p className="text-[#2a4558] text-xs">
                {lang === "es" ? `Tienes ${currentDisplay}${isPct ? "%" : ""}, máximo ${maxDisplay}${isPct ? "%" : ""}`
                  : `You have ${currentDisplay}${isPct ? "%" : ""}, max ${maxDisplay}${isPct ? "%" : ""}`}
              </p>
            </div>
          )}
        </div>
      ...
      {hasSolutions ? (
        <div className="space-y-4">
          ...
          <SolutionCard
            ...
            attr={selectedAttr} isPct={isPct}
            targetValue={internalTarget} currentStats={currentStats}
            lang={lang} builderMode={builderMode}
...
function SolutionCard({
  ...
  lang: Lang; builderMode: "total" | "add";
  openerPrice: number; includeOpener: boolean;
  checkedKeys: Set<string>; checkMode: "mark-only" | "update-rank";
    ...
    <div className={`p-4 rounded-lg border ${bgColor} ${borderColor}`}>
      ...
      <div className="grid grid-cols-2 gap-2 mb-4">
        <div className="bg-black/30 p-2 rounded">
          ...
          <p className="text-white font-semibold text-sm">{formatM(solution.totalCost)}</p>
        </div>
        <div className="bg-black/30 p-2 rounded">
          <p className="text-[#5a8aaa] text-xs font-mono uppercase">{lang === "es" ? "Sellos" : "Seals"}</p>
          <p className="text-white font-semibold text-sm">{solution.totalSeals.toLocaleString()}x</p>
        </div>
        {totalOpeners > 0 && (
          <div className="bg-black/30 p-2 rounded col-span-2 flex items-center justify-between">
            <p className="text-[#5a8aaa] text-xs font-mono uppercase">📦 Openers</p>
            <p className="text-[#ffd700] font-bold text-sm">{totalOpeners}x</p>
          </div>
        )}
      </div>
      ...
      <div className="mb-4 pb-3 border-b border-current border-opacity-30">
        ...
        {targetValue > 0 && (
          <div className="w-full h-1.5 bg-black/30 rounded overflow-hidden">
            <div className={`h-full rounded transition-all ${barColor}`} style={{ width: `${progressPct}%` }} />
          </div>
        )}
      </div>
      ...
      <div className="space-y-1.5 text-xs">
          ...
          <div className="flex items-center gap-2">
            ...
            {totalCount > 0 && (
              <span className={`text-[10px] font-mono px-1.5 py-0.5 rounded-full border ${
                allDone ? "border-[#00e676] text-[#00e676] bg-[#00e676]/10" : "border-[#1a3f6e] text-[#5a8aaa]"
              }`}>{checkedCount}/{totalCount}</span>
            )}
            {checkedCount > 0 && (
              <button onClick={onClearChecks} className="text-[#2a4558] text-[10px] font-mono hover:text-[#5a8aaa] transition-colors">
                ✕ {lang === "es" ? "limpiar" : "clear"}
              </button>
            )}
          </div>

          {/* Toggle modo mark-only / update-rank */}
            ...
            {(["mark-only", "update-rank"] as const).map(m => (
              <button key={m} onClick={() => onCheckModeChange(m)}
                className={`px-2 py-1 rounded text-[10px] font-mono transition-all border ${
                  checkMode === m
                    ? m === "update-rank" ? "bg-[#00e676]/15 border-[#00e676] text-[#00e676]" : "bg-[#00c8f0]/15 border-[#00c8f0] text-[#00c8f0]"
                    : "border-[#1a3f6e] text-[#5a8aaa] hover:border-[#5a8aaa]"
                }`}>
                {m === "mark-only"
        ...
        {checkMode === "update-rank" && (
          <div className="flex items-start gap-1.5 p-2 rounded bg-[#00e676]/08 border border-[#00e676]/20 mb-2">
            ...
            <p className="text-[#00e676] text-[10px] font-mono">
              {lang === "es"
                ? "Al marcar un sello se actualizará automáticamente tu rank en Gestión. Úsalo solo después de abrir y subir el rank en el juego."
                : "Checking a seal will automatically update your rank in Manage. Use only after opening and upgrading in-game."}
            </p>
          </div>
        )}
          ...
          {(["efficiency", "price", "order"] as const).map(s => (
            <button key={s} onClick={() => setSortType(s)}
              className={`px-2 py-1 rounded text-[10px] font-mono transition-all ${
                sortType === s ? "bg-[#00c8f0]/30 border border-[#00c8f0] text-[#00c8f0]" : "border border-[#1a3f6e]/50 text-[#5a8aaa] hover:border-[#00c8f0]/50 hover:text-[#00c8f0]"
              }`}>
          ...
          <div className="flex items-center gap-2 p-2 rounded-lg bg-[#00e676]/10 border border-[#00e676]/40 mb-2">
            ...
            <span className="text-[#00e676] font-mono text-[11px] font-bold">
              {checkMode === "update-rank"
                ...
                : (lang === "es" ? "¡Todo comprado! Ya puedes subir tus sellos." : "All bought! You can now upgrade your seals.")}
            </span>
          </div>
        )}
          ...
          return (
            <div key={key} onClick={() => onToggleCheck(key, item.name, item.rank as Rank)}
              ...
              <div className={`mt-0.5 w-4 h-4 rounded border-2 flex items-center justify-center shrink-0 transition-all ${
                isDone ? "border-[#00e676] bg-[#00e676]" : "border-[#1a3f6e] hover:border-[#5a8aaa]"
              }`}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[#5a8aaa] text-[10px] mt-0.5">
                  <span className="px-1.5 py-0.5 rounded text-[10px] font-bold mr-1" style={{
                    color: RANK_COLOR[item.rank as Rank] ?? "#fff",
                    background: `${RANK_COLOR[item.rank as Rank] ?? "#fff"}20`,
                    border: `1px solid ${RANK_COLOR[item.rank as Rank] ?? "#fff"}40`,
                  }}>{item.rank}</span>
              ...
              <div className="text-right shrink-0">
                <p className={`font-semibold text-xs ${isDone ? "text-[#2a4558]" : "text-[#00c8f0]"}`}>
                  +{isPct ? formatStat(attr, item.statBonus) : item.statBonus.toLocaleString()}
                </p>
                <p className="text-[#5a8aaa] text-[10px]">{formatM(item.totalCostM)}</p>
                {efficiency > 0 && sortType === "efficiency" && (
                  <p className="text-[#2a4558] text-[10px] font-mono">{formatM(efficiency)}/stat</p>
                )}
              </div>
            </div>
          );
        ...
        {includeOpener && openerPrice > 0 && (
          <p className="text-[#2a4558] text-[10px] font-mono text-center mt-2">
            💡 {lang === "es" ? "Incluye opener:" : "Includes opener:"} {formatM(openerPrice / 50)}/sello
          </p>
        )}
      </div>
    </div>
