// ============================================================
//  ManageTab.tsx  —  Gestionar sellos
//  Mejoras:
        ...
        <div>
          ...
          <div className="flex items-start gap-2 p-2 rounded-lg bg-[#00c8f0]/08 border border-[#00c8f0]/20">
            ...
            <p className="text-[#00c8f0] text-xs font-mono leading-relaxed">
              {lang === "es"
                ? "Este es TU rank personal — el rank al que ya subiste este sello en tu personaje. No afecta a otros jugadores."
                : "This is YOUR personal rank — the rank you've already upgraded this seal to on your character. Does not affect other players."}
            </p>
          </div>
        </div>
...
export function ManageTab({
  ...
  const seals = useMemo(() => {
    ...
    if (sealTypeFilter === "normal") list = list.filter(s => !isDMONoMarket(s.name));
    if (sealTypeFilter === "event")  list = list.filter(s => isDMONoMarket(s.name));
    return [...list].sort((a, b) => {
      // Nuevas primero, luego evento, luego el sort elegido
      const aNew   = isDMONew(a.name)   ? 0 : 1;
      const bNew   = isDMONew(b.name)   ? 0 : 1;
      const aEvent = isDMONoMarket(a.name) ? 0 : 1;
      const bEvent = isDMONoMarket(b.name) ? 0 : 1;
      if (aNew   !== bNew)   return aNew   - bNew;
      if (aEvent !== bEvent) return aEvent - bEvent;
      if (sortKey === "name-asc")  return a.name.localeCompare(b.name);
      if (sortKey === "name-desc") return b.name.localeCompare(a.name);
      const getMax = (s: typeof a) => attrFilter ? (s.stats?.[attrFilter]?.["Master"] ?? 0) : 0;
  ...
  const resetAllToUnopened = () => {
    ...
    onUpdate({ ...data, seals: Object.fromEntries(Object.entries(data.seals).map(([k, s]) => [k, { ...s, currentRank: "Unopened" as Rank }])), lastUpdated: Date.now() });
  };

  const deleteSeal = (name: string) => {
    if (!confirm(t.confirmDelete(name))) return;
    const { [name]: _, ...rest } = data.seals;
    ...
    <div>
        ...
        <div className="flex flex-wrap gap-1.5 flex-1">
          <button onClick={() => handleAttrFilter(null)}
            className={`px-3 py-1 rounded-full text-xs font-mono border transition-all ${
              attrFilter === null ? "border-[#00c8f0] text-[#00c8f0] bg-[#00c8f0]/15" : "border-[#1a3f6e] text-[#5a8aaa] hover:border-[#00c8f0]/50 hover:text-white"
            }`}>{t.filterAll}</button>
          {ATTRIBUTES.map(attr => (
            <button key={attr} onClick={() => handleAttrFilter(attrFilter === attr ? null : attr)}
              className={`px-3 py-1 rounded-full text-xs font-mono border transition-all flex items-center gap-1 ${
                attrFilter === attr ? "border-[#00c8f0] text-[#00c8f0] bg-[#00c8f0]/15" : "border-[#1a3f6e] text-[#5a8aaa] hover:border-[#00c8f0]/50 hover:text-white"
              }`}>
            ...
            return (
              <button key={opt.key} onClick={() => !disabled && onSortKeyChange(opt.key)} disabled={disabled}
                className={`px-2.5 py-1 rounded text-xs font-mono border transition-all ${
                  sortKey === opt.key ? "border-[#00c8f0] text-[#00c8f0] bg-[#00c8f0]/15"
                  : disabled ? "border-[#1a3f6e]/40 text-[#2a4558]/40 cursor-not-allowed"
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
        {seals.map(seal => {
          ...
          const isNew     = isDMONew(seal.name);
              ...
              <div className="p-3">
                ...
                <div className="mb-1 h-5 flex items-center gap-1">
                  ...
                  {rank
                    ...
                    : <span className="text-[#2a4558] text-[10px] font-mono">{lang === "es" ? "sin rank" : "no rank"}</span>}
                </div>
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    {seal.priceM > 0 ? (
                      <span className="text-[10px] font-mono">
                        <span className={age === "stale" ? "text-orange-400" : "text-[#00c8f0]"}>
                          {formatM(seal.priceM)}
                        </span>
                    ...
                    {age === "stale" && (
                      <span className="text-orange-400 text-[9px]" title={lang === "es" ? "Precio desactualizado (+7 días)" : "Stale price (+7 days)"}>⚠</span>
                    )}
                  </div>
                  {ts && seal.priceM > 0 && (
                    <p className={`text-[9px] font-mono ml-3 ${age === "stale" ? "text-orange-400/70" : "text-[#2a4558]"}`}>
