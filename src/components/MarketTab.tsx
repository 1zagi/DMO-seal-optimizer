// ============================================================
//  MarketTab.tsx  —  Vista de tendencias de mercado
//  Para traders: detecta sellos con precio al alza, a la baja,
//  y sin historial reciente (posible escasez o precio sin fijar)
// ============================================================
...
interface TrendEntry {
  sealId:    string;
  current:   number;
  previous:  number;        // precio hace ~7 días
  change:    number;        // diferencia absoluta
  changePct: number;        // % de cambio
  trend:     "up" | "down" | "stable" | "new"; // "new" = sin historial previo
}
...
export function MarketTab({ data, lang, fetchPricesNDaysAgo, fetchPriceHistory }: Props) {
  ...
  // ── Cargar historial de sello seleccionado ────────────────
  useEffect(() => {
    ...
    <div className="space-y-4">
      <div className="space-y-1">
        ...
        <p className="text-[#5a8aaa] font-mono text-sm">
          {lang === "es" ? "Tendencias de precio vs hace 7 días" : "Price trends vs 7 days ago"}
        </p>
      </div>
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          ...
          <button key={key}
            ...
            style={{
              borderColor: filter === key ? color : `${color}30`,
              background:  filter === key ? `${color}15` : "#09141f",
            }}>
          ...
          {([
            ...
            <button key={k} onClick={() => setSortKey(k)}
              className={`px-3 py-2 rounded text-xs font-mono border transition-all ${
                sortKey === k ? "border-[#00c8f0] text-[#00c8f0] bg-[#00c8f0]/10" : "border-[#1a3f6e] text-[#5a8aaa] hover:border-[#5a8aaa]"
              }`}>{label}</button>
          ))}
        </div>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-[#09141f] border-b border-[#1a3f6e]">
              {[
                ...
                lang === "es" ? "Hace 7 días"   : "7 days ago",
          ...
          <tbody>
            {displayed.map(t => {
              ...
              const color    = isUp ? "#00e676" : isDown ? "#ff6b6b" : isNew ? "#ffd700" : "#5a8aaa";
              const icon     = isUp ? "↑" : isDown ? "↓" : isNew ? "?" : "–";
              const isSelected = selectedSeal === t.sealId;

              return (
                <tr key={t.sealId}
                  onClick={() => setSelectedSeal(s => s === t.sealId ? null : t.sealId)}
                  className={`border-b border-[#1a3f6e]/50 cursor-pointer transition-all ${
                    isSelected ? "bg-[#1a3f6e]/40" : "hover:bg-[#1a3f6e]/20"
                  }`}>
                  ...
                  <td className="px-4 py-3 text-right font-mono text-[#5a8aaa]">
                    {isNew ? <span className="text-[#ffd700] text-xs">sin datos</span> : formatM(t.previous)}
                  </td>
                  <td className="px-4 py-3 text-right font-mono font-bold" style={{ color }}>
                    {isNew ? "–" : `${t.change >= 0 ? "+" : ""}${formatM(Math.abs(t.change))}`}
                  </td>
                  <td className="px-4 py-3 text-right font-mono font-bold" style={{ color }}>
                    {isNew ? "–" : `${t.changePct >= 0 ? "+" : ""}${t.changePct.toFixed(1)}%`}
                  </td>
            ...
            {displayed.length === 0 && (
              <tr><td colSpan={6} className="px-4 py-10 text-center text-[#5a8aaa] font-mono text-sm">
      ...
      {/* Panel de historial del sello seleccionado */}
      {selectedSeal && (
        <div className="p-4 bg-[#09141f] border border-[#1a3f6e] rounded-xl">
          ...
          {histLoading ? (
            ...
            <p className="text-[#5a8aaa] font-mono text-sm">
              {lang === "es" ? "Sin historial en los últimos 30 días. Los precios se registran cuando alguien los actualiza." : "No history in the last 30 days. Prices are recorded when someone updates them."}
            </p>
          ) : (
            <>
                ...
                {[...history].reverse().map((p, i) => {
                  ...
                  return (
                    <div key={i} className="flex items-center justify-between px-2 py-1 rounded text-xs font-mono">
                      ...
                      {delta !== 0 && (
                        <span style={{ color: delta > 0 ? "#00e676" : "#ff6b6b" }}>
                          {delta > 0 ? "+" : ""}{formatM(Math.abs(delta))}
                        </span>
                      )}
                    </div>
                  );
      ...
      <p className="text-[#2a4558] text-xs font-mono">
        {lang === "es"
          ? "💡 Los sellos marcados con ? no tienen registro de hace 7 días — pueden ser nuevos, o nadie había puesto precio antes."
          : "💡 Seals marked with ? have no record from 7 days ago — they may be new, or no one had set a price before."}
      </p>
    </div>
  );
}
...
// ── Mini sparkline SVG ────────────────────────────────────────
function MiniChart({ points }: { points: { price_m: number; recorded_at: string }[] }) {
  ...
  const color = isUp ? "#00e676" : "#ff6b6b";
