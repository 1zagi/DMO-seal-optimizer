 ...
 *
 * dp[u].score  = mejor puntaje acumulado para llegar exactamente a u unidades
 * dp[u].prevU  = índice u del que venimos (para reconstruir la solución)
 * dp[u].optIdx = índice dentro de `allOpts` que elegimos al llegar a u
 * dp[u].sealIdx = índice del grupo de sello usado
 *
 * Al terminar, reconstruimos los items haciendo traceback desde targetUnits → 0.
 */
function knapsack(
  candidates: Candidate[],
  targetStats: number,
  ...
  const usedSeal = new Int16Array(targetUnits + 1).fill(-1);  // índice del sealGroup
  const usedOpt  = new Int16Array(targetUnits + 1).fill(-1);  // índice dentro del grupo
