// ============================================================
//  supabase.ts  —  Cliente Supabase para DMO
// ============================================================
...
export type ServerId = "omegamon" | "alphamon";
...
export async function fetchServerPrices(serverId: ServerId): Promise<Map<string, ServerPriceEntry>> {
  try {
    const res = await fetch(
      `${SUPABASE_URL}seal_prices?server_id=eq.${serverId}&select=seal_id,price_m,updated_at`,
      { headers: headers() }
    );
...
/**
 * Upsert precio actual + inserta en historial.
 * Solo inserta en historial si el precio cambió respecto al actual.
 */
export async function upsertSealPrice(
  serverId: ServerId,
  sealId: string,
  priceM: number,
  prevPriceM?: number,   // precio anterior para evitar duplicados en historial
): Promise<void> {
  ...
  try {
    ...
    const res = await fetch(`${SUPABASE_URL}seal_prices`, {
      method:  "POST",
      headers: headers(),
      body: JSON.stringify({ server_id: serverId, seal_id: sealId, price_m: priceM, updated_at: now }),
    });
    if (!res.ok) throw new Error(await res.text());

    // 2. Insertar en historial solo si el precio cambió
    if (prevPriceM === undefined || prevPriceM !== priceM) {
      await insertPriceHistory(serverId, sealId, priceM, now);
    }
  } catch (err) {
...
async function insertPriceHistory(
  serverId: string, sealId: string, priceM: number, recordedAt: string
): Promise<void> {
  try {
    const res = await fetch(`${SUPABASE_URL}seal_price_history`, {
      method:  "POST",
      headers: headersInsert(),
      body: JSON.stringify({ server_id: serverId, seal_id: sealId, price_m: priceM, recorded_at: recordedAt }),
    });
...
/**
 * Obtiene el historial de precio de un sello específico.
...
/**
 * Obtiene el último precio registrado en el historial para todos los sellos
 * de un servidor hace N días — para calcular tendencias en masa.
 * Devuelve Map<sealId, price_m_hace_N_dias>
 */
export async function fetchPricesNDaysAgo(
  serverId: ServerId,
  days = 7,
): Promise<Map<string, number>> {
  try {
    // Busca el precio más reciente registrado ANTES de hace N días
    // Ventana amplia: desde hace 60 días hasta hace N-1 días
    ...
    // Solo el más reciente por sello (ya viene ordenado desc)
    const map = new Map<string, number>();
    for (const r of rows) {
      if (!map.has(r.seal_id)) map.set(r.seal_id, r.price_m);
    }
    return map;
  } catch (err) {
...
export function subscribeToServerPrices(
  ...
  const connect = () => {
    ...
    ws.onopen = () => {
      ws?.send(JSON.stringify({ topic: `realtime:public:seal_prices:server_id=eq.${serverId}`, event: "phx_join", payload: {}, ref: "1" }));
    ...
    ws.onmessage = (evt) => {
      try {
        ...
        if (msg.event === "INSERT" || msg.event === "UPDATE" || msg.payload?.type === "INSERT" || msg.payload?.type === "UPDATE") {
          ...
          if (record?.server_id === serverId && record.seal_id) {
