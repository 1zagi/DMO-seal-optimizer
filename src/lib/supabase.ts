...
export async function fetchServerPrices(serverId: ServerId): Promise<Map<string, number>> {
  try {
    const res = await fetch(
      `${SUPABASE_URL}seal_prices?server_id=eq.${serverId}&select=seal_id,price_m`,
      { headers: headers() }
    );
    ...
    ws.onmessage = (evt) => {
      try {
        ...
        if (msg.event === "INSERT" || msg.event === "UPDATE" || msg.payload?.type === "INSERT" || msg.payload?.type === "UPDATE") {
          ...
          if (record?.server_id === serverId && record.seal_id) onUpdate(record.seal_id, record.price_m);
        }
      } catch { }
    };
